...
En **servidor**, el orden de carga es: `KITEPROP_PROPERTIES_JSON_URL` (JSON del export) → si no hay URL, **API** `GET …/properties` con `KITEPROP_API_KEY` / `KITEPROP_API_TOKEN` como **`X-API-Key`** → si falla, snapshot `properties.json` del repo. Mapeo API→`RawProperty`: `kiteprop-api-mapper`. Tabla de variables: **`docs/KITEPROP.md`**; doc oficial: [API v1](https://www.kiteprop.com/docs/api/v1).
