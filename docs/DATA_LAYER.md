...
En **servidor**, el orden de carga es: `KITEPROP_PROPERTIES_JSON_URL` (JSON del export) → si no hay URL, **API** `GET …/properties` con `KITEPROP_API_KEY` / `KITEPROP_API_TOKEN` como **`X-API-Key`** → si falla, snapshot `properties.json` del repo. Mapeo API→`RawProperty`: `kiteprop-api-mapper` + `enrichRawPropertyFromKitepropAliases` (alias ampliados: `difusion_tags`, `web_tags`, etc.; **`enrich` recibe la fila original del feed** además del objeto mapeado, para que un JSON con forma “API” no pierda etiquetas que el mapper no copia al `RawProperty`). Luego `mergePremierMetadataFromRepoSnapshot` puede reponer tags vacíos desde el snapshot por `id`. Tabla de variables: **`docs/KITEPROP.md`**; doc oficial: [API v1](https://www.kiteprop.com/docs/api/v1).
