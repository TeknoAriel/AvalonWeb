...
- Si **GitHub dejó de disparar deploys** en Vercel sin tocar el código, el repo incluye **GitHub Actions** que publican con la CLI: `.github/workflows/deploy-avalon-premier.yml` y `deploy-avalon-propiedades.yml`. Hacen falta secrets en el repo: **`VERCEL_TOKEN`**, **`VERCEL_ORG_ID`**, **`VERCEL_PROJECT_ID`** (Premier) y opcional **`VERCEL_PROJECT_ID_PROPIEDADES`**. Token: [Vercel → Account → Tokens](https://vercel.com/account/tokens); IDs del proyecto en **Settings → General**.
