...
- **Deploy desde GitHub** (si la integración Git→Vercel no dispara builds): workflow **`.github/workflows/deploy-vercel.yml`**. Un solo secret **`VERCEL_TOKEN`** ([crear token](https://vercel.com/account/tokens)). El workflow hace **`vercel link` → `pull` → `build --prod` → `deploy --prebuilt --prod`** desde la raíz del repo con `--cwd` a cada app (monorepo). Sin `--git-config=false` en `pull` (evita fallos con el checkout de Actions). Los slugs y el team están en el YAML (`matrix` + `VERCEL_TEAM`); si falla el link del sitio principal, ajustá el `project` del matrix al slug exacto (URL `vercel.com/<team>/<slug>`).
