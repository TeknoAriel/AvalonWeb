...
| Variable | Uso |
|----------|-----|
| `KITEPROP_PROPERTIES_JSON_URL` | JSON de difusión (**respaldo** si no hay API key o la API no devuelve filas). Opcional si ya usás solo API. |
| `KITEPROP_API_KEY` o `KITEPROP_API_TOKEN` | `X-API-Key` en REST. **Con key configurada**, el catálogo intenta **primero** `GET …/properties` (origen más fiel al CRM y a tags Premier); el JSON solo si eso falla o está vacío. |
