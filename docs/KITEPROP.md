...
- **Deploy desde GitHub** (si la integración Git→Vercel no dispara builds): workflow **`.github/workflows/deploy-vercel.yml`**. Un solo secret **`VERCEL_TOKEN`** ([crear token](https://vercel.com/account/tokens)). El workflow hace **`vercel link` → `pull` → `build --prod` → `deploy --prebuilt --prod`** en cada carpeta `apps/*` (coincide con `vercel.json`). Sin `--git-config=false` en `pull`. Si falla **`vercel build`**, en el run de Actions aparece un artefacto **`vercel-build-log-<label>`** con el log del CLI. Slugs y team: `matrix` + `VERCEL_TEAM`; si falla el link del sitio principal, ajustá `project` al slug exacto (URL `vercel.com/<team>/<slug>`).
