...
- **Deploy desde GitHub:** workflow **`.github/workflows/deploy-vercel.yml`** — solo **`avalon-premier`**: un **`vercel deploy --prod`** con `--cwd apps/avalon-premier` (build remoto en Vercel). Secrets **`VERCEL_TOKEN`**, **`VERCEL_ORG_ID`**, **`VERCEL_PROJECT_ID`** (IDs en Vercel → team / proyecto → Settings). Sin `vercel link` en CI. La web principal (`avalonweb`) sigue con **Git → Vercel**.
