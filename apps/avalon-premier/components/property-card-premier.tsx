...
export function PropertyCardPremier({ property }: { property: NormalizedProperty }) {
  const img = property.media.images[0];
  return (
    <Link
      href={`/propiedades/${property.slug}`}
      className="group block overflow-hidden bg-brand-surface-alt/40 transition hover:bg-brand-surface-alt/70"
    >
